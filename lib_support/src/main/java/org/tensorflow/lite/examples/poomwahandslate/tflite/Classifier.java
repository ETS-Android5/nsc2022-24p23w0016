...
package org.tensorflow.lite.examples.poomwahandslate.tflite;
...
public abstract class Classifier {
  ...
  public static Classifier create(Activity activity, Model model, Device device, int numThreads)
    ...
    } else if (model == Model.EN1_AM) {
      return new ClassifierEN1(activity, device, numThreads);
    } else if (model == Model.EN2_NY) {
      return new ClassifierEN2(activity, device, numThreads);
    } else if (model == Model.TH1_กถ) {
      return new ClassifierTH1(activity, device, numThreads);
    } else if (model == Model.TH2_ทฮ) {
      return new ClassifierTH2(activity, device, numThreads);
	} else if (model == Model.QUANTIZED_DEFAULT) {
